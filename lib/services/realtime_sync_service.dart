...
class RealtimeSyncService {
  ...
  final StreamController<List<Map<String, dynamic>>> _alertsController = StreamController<List<Map<String, dynamic>>>.broadcast();
  final StreamController<DigitalTouristID?> _digitalIdController = StreamController<DigitalTouristID?>.broadcast();
  final StreamController<List<EmergencyContact>> _emergencyContactsController = StreamController<List<EmergencyContact>>.broadcast();
  ...
  Stream<List<Map<String, dynamic>>> get alertsStream => _alertsController.stream;
  Stream<DigitalTouristID?> get digitalIdStream => _digitalIdController.stream;
  Stream<List<EmergencyContact>> get emergencyContactsStream => _emergencyContactsController.stream;
  ...
  Future<void> _setupConnectivityMonitoring() async {
    ...
    _subscriptions['connectivity'] = Connectivity().onConnectivityChanged.listen((result) {
  ...
  Future<void> _setupDataStreams(String userId) async {
    // User profile stream
    _subscriptions['userProfile'] = _firestoreService.streamUserProfile(userId).listen(
      (userData) {
    ...
    // Alerts stream
    _subscriptions['alerts'] = _firestoreService.streamUserAlerts(userId).listen(
      (alerts) {
    ...
    // Digital ID stream
    _subscriptions['digitalId'] = _firestoreService.streamDigitalId(userId).listen(
      (digitalId) {
    ...
    _subscriptions['emergencyContacts'] = _firestoreService.streamEmergencyContacts(userId).listen(
      (contacts) {
  ...
  Future<void> _setupLocationTracking(String userId) async {
    try {
      ...
      _subscriptions['location'] = _locationService.getPositionStream().listen(
        (position) {
          _locationController.add(position);
          _syncLocationToFirestore(userId, position);
        },
        onError: (error) {
  ...
  Future<void> _syncCurrentLocation(String userId) async {
    try {
      ...
      if (position != null) {
        await _syncLocationToFirestore(userId, position);
      }
    } catch (e) {
  ...
  Future<void> _syncLocationToFirestore(String userId, Position position) async {
    final locationData = {
    ...
    if (_isOnline) {
      try {
        await _firestoreService.saveLocation(userId, locationData);
      } catch (e) {
  ...
  Future<void> _processPendingOperations() async {
    ...
    for (final operation in operationsToProcess) {
      try {
        ...
        print('Failed to process pending operation: ${operation.type}, error: $e');
